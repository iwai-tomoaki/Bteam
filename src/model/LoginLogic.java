...
public class LoginLogic extends Division_DAO{
	...
	public LoginLogic(User user){
		...
		int roop = user_num.size();			//DivisionDAOで取得したuser_intの行数(要素数)をroop変数に代入
		for(int roop_order = 0; roop_order < roop; roop_order ++) {		//取得した要素数分ループ
			int user_num_order = user_num.get(roop_order);			//ArrayList<Integer>の(roop_order)番目の中身をint型に変換してuser_num_orderに代入
			String user_pass_order = user_pass.get(roop_order);	//ArrayList<String>の(roop_order)番目の中身をString型に変換してuser_pass_orderに代入
